...
def rayleigh(z,sigma, H):
    p1=1/(math.sqrt(2*math.pi*sigma**2))*(math.exp(-(z-H)**2/(2*sigma**2)))
    p0 = 1/(math.sqrt(2*math.pi*sigma**2))*(math.exp(-(z+H)**2/(2*sigma**2)))
...
def rayleigh_channel(s,SNRdB, L=1):
    ...
    # h = standard_normal(s.shape)+1j*standard_normal(s.shape)
    y,r,r0 = [],[],[]
...
for ind_1 in range(len(SNR)):
    ...
    for ind_2 in range(BER_batch):
        ...
        sigma, received, modH = rayleigh_channel(bpsk_encoded, SNR[ind_1])
        #print(bpsk_encoded, received)
        received = received.flatten()
        #print("Received signal is {}".format(received))
        # print(received)
        # exit()
        intrinsic = [rayleigh(received[i], sigma, modH[i]) for i in range(len(received))] # to be discussed
        # print(intrinsic)
        # exit()
